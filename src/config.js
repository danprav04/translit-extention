...
let localEnv = {};
try {
  // Dynamically import config.local.js if generated by sync-env.js
  const mod = await import('./config.local.js');
  localEnv = mod.LOCAL_ENV || {};
} catch (err) {
  console.warn('[config.js] config.local.js not found or not loadable. Using default fallbacks.');
}

export const DEFAULT_CONFIG = {
  apiKey: localEnv.GEMINI_API_KEY || "",
  primaryModel: localEnv.PRIMARY_MODEL || "gemini-3.1-flash-lite-preview",
  fallbackModel: localEnv.FALLBACK_MODEL || "gemma-4-31b-it",
